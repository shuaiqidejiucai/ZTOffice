#ifndef PST_ROUNDTRIPOARTTEXTSTYLES12ATOM_H
#define PST_ROUNDTRIPOARTTEXTSTYLES12ATOM_H

#include "pst_base.h"

class PSTParser_EXPORT PST_RoundTripOArtTextStyles12Atom : public PST_Base
{
public:
    PST_RoundTripOArtTextStyles12Atom(const QByteArray& srcData, const ST_Variable& var);

    virtual int parser(PSTSearch* pSearchPtr = nullptr)override;

    virtual void clearParserData()override;

    virtual int priority()override;
};

#endif // PST_ROUNDTRIPOARTTEXTSTYLES12ATOM_H
